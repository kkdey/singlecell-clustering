---
title: "Pantaleo exploratory analysis - TFH041 + TFH146 pooled"
author: "Kushal K Dey"
date: "8/8/2017"
...
TFH_LN_reads <- read.table('../data/TFH041/TFH Sorted/LN/reads.TFH041_TFH_LN_S1_R1_001.fastq_bq10_star_corrected.reads.dge.txt.gz', header = TRUE, row.names = 1)
...
```{r echo=FALSE, eval=FALSE}
TFH_fac_1 <- c(rep("TFH041_LN", dim(TFH_LN_UMI_filtered)[2]),
                   rep("TFH041_PBMC", dim(TFH_PBMC_UMI_filtered)[2]))
length(TFH_fac_1)
```
...
```{r echo=FALSE, eval=FALSE}
TFH_fac_2 <- c(rep("TFH146_LN", dim(TFH_LN_UMI_filtered)[2]),
                   rep("TFH146_PBMC", dim(TFH_PBMC_UMI_filtered)[2]))
length(TFH_fac_2)
```

```{r echo=FALSE, eval=FALSE}
counts <- rbind.fill(as.data.frame(t(TFH_UMI_1)), as.data.frame(t(TFH_UMI_2)))
rownames(counts) <- c(paste0(colnames(TFH_UMI_1), "_041"), paste0(colnames(TFH_UMI_2), "_146"))
```
...
```{r echo=FALSE, eval=FALSE}
save(seuratObj_TFH_global, file = "../output/seurat_TFH_TFH041_TFH146_pooled.rda")
```

```{r echo=FALSE, eval=FALSE}
TFH_fac <- c(TFH_fac_1, TFH_fac_2)
save(TFH_fac, file = "../output/fac_TFH_TFH041_TFH146_pooled.rda")
```


```{r echo=FALSE, eval=TRUE}
seuratObj_TFH_global <- get(load("../output/seurat_TFH_TFH041_TFH146_pooled.rda"))
fac <- get(load("../output/fac_TFH_TFH041_TFH146_pooled.rda"))
```
...
save(topic_clus_list, file = "../output/CountClust_Model_TFH041_TH146_TF.rda")
```
...
```{r echo=FALSE, eval=TRUE}
topic_clus <- get(load("../output/CountClust_Model_TFH041_TH146_TF.rda"))
```
...
```{r echo=FALSE, eval=TRUE}
indices <- ExtractTopFeatures(topic_clus[[4]]$theta, top_features = 100, method = "poisson", options = "max")
imp_genes <- apply(indices, 1, function(x) return(rownames(seuratObj_TFH_global@data)[x]))
