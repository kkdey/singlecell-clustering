...
```{r}
idx1 <- which(tissue_names == "Blood Vessel")
mat2 <- mat[, idx1]
```
...
topic_clus <- classtpx::class_topics(pooled_data, K=9, known_samples = known_samples, class_labs = class_labs_2, method = "theta.fix", shrink = FALSE, tol = 0.001)
save(topic_clus, file = "../output/classtpx_gtex_blood_vessel_sorted_immune_thresh.rda")
```

## Cluster representation

```{r}
topic_clus <- get(load("../output/classtpx_gtex_blood_vessel_sorted_immune_thresh.rda"))
```
...
```{r echo=FALSE, eval=TRUE}
labs <- c(as.character(fac1), rep("Blood-vessel", 689))
PlotCountClust(topic_clus, labs)
```


```{r echo=FALSE, eval=FALSE}
topic_clus <- classtpx::class_topics(pooled_data, K=11, known_samples = known_samples, class_labs = class_labs_2, method = "theta.fix", shrink = FALSE, tol = 100)
save(topic_clus, file = "../output/classtpx_gtex_blood_vessel_sorted_immune_thresh_2.rda")
```


```{r}
topic_clus_2 <- get(load("../output/classtpx_gtex_blood_vessel_sorted_immune_thresh_2.rda"))
```
