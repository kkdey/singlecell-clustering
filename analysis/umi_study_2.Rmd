...
```{r}
out <- getBM(attributes = c('external_gene_name', 'transcription_start_site'),
...
```{r}
mat <- data.frame(tbl_df(out) %>% group_by(external_gene_name) %>% 
  summarise(n = n()))
```
...
```{r}
mat[order(mat$n, decreasing = TRUE)[1:50],]
```
...
```{r}
plot(mat2$medianUMI, mat2$n, ylab = "Number of Transcription start sites (gene)",
...
```{r}
plot(mat2$meanUMI, mat2$n, ylab = "Number of Transcription start sites (gene)",
...
```{r}
mat[order(mat$n, decreasing = TRUE)[1:10],]
```
...
```{r}
idx <- match(mat$external_gene_name, rownames(LiveCells_UMI))
idx2 <- idx[!is.na(idx)]
